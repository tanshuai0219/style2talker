...
from model.motion_driven.refinement import CalibrationNet
from model.motion_driven.motiongenerator import Motion_Generator
...
class Audio_Driven_VToonify(nn.Module):
    def __init__(self,
                 in_size=256,
                 out_size=1024,
                 img_channels=3,
        ...
        self.refinement = CalibrationNet(
            out_size=64,
            input_channel=512, 
            num_style_feat=512,
            channel_multiplier=2,
    ...
    def forward(self, x, style, input_image, driven_3dmm, d_s=None, return_mask=False, return_feat=False):
        # map style to W+ space
        if style is not None and style.ndim < 3:
            if self.backbone == 'dualstylegan':
                resstyles = self.generator.style(style).unsqueeze(1).repeat(1, self.generator.n_latent, 1)
        ...
        video_output = self.motion_generator(input_image, driven_3dmm)  # Input: 256*256 Output(flow_field torch.Size([10, 2, 64, 64]); warp_image:torch.Size([10, 3, 256, 256]) 'descriptor':# torch.Size([10, 256, 1])) 
        flow = video_output['flow_field'] # torch.Size([10, 2, 64, 64])
        descriptor = video_output['descriptor'] # torch.Size([10, 256, 1])
        video_warping_condition = flow_util.convert_flow_to_deformation(flow) # torch.Size([10, 64, 64, 2])
        ...
        for conv1, conv2, to_rgb in zip(
            ...
            out = conv1(out, adastyles[:, _index+6], noise=noise)
            # print(out.shape)
            if i == 7: # torch.Size([1, 512, 72, 72])
                refining_condition = self.refinement(out, descriptor)
