...
class FaceReconModel(BaseModel):
    ...
    def forward(self):
        ...
        self.pred_vertex, self.pred_tex, self.pred_color, self.pred_lm = \
            self.facemodel.compute_for_render(output_coeff) # face_vertex, face_texture, face_color, landmark
        self.pred_mask, _, self.pred_face = self.renderer(
            self.pred_vertex, self.facemodel.face_buf, feat=self.pred_color)
        self.save_img(self.pred_mask, 'pred_mask.png')
        self.save_img(self.pred_face, 'pred_face.png')
        self.pred_coeffs_dict = self.facemodel.split_coeff(output_coeff)
        self.output_coeff = output_coeff


    def change_1dim(self):
        output_coeff = self.net_recon(self.input_img) # torch.Size([1, 257])
        for i in range(80, 144):

            cur = output_coeff.clone().detach()
            for j in range(80, 144):
                cur[:, j] = 0
            cur[:, i] = 10
            self.facemodel.to(self.device)
            self.pred_vertex, self.pred_tex, self.pred_color, self.pred_lm = \
                self.facemodel.compute_for_render(cur) # face_vertex, face_texture, face_color, landmark
            self.pred_mask, _, self.pred_face = self.renderer(
                self.pred_vertex, self.facemodel.face_buf, feat=self.pred_color)
            
            self.save_img(self.pred_mask, 'pred_mask.png')
            self.save_img(self.pred_face, 'pred_face_'+str(i)+'.png')
    ...
    def save_img(self, img, path):
        image_numpy = util.tensor2im(img[0])
