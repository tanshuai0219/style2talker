...
from util.tensor_utils import convert_to_tensor, convert_to_np

with open("reconstructor_opt.pkl", "rb") as f:
    opt = pkl.load(f) 
opt.epoch = '20'
opt.name = 'face_recon'
class Reconstructor(nn.Module):
    ...
    @torch.no_grad()
    def recon_coeff_vis(self, batched_images, batched_lm5, return_image=True, batch_mode=True):
        bs = batched_images.shape[0]
        data_lst = []
        for i in range(bs):
            img = batched_images[i]
        ...
        if not batch_mode:
            ...
            self.model.set_input(data)  # unpack data from data loader
            self.model.forward()
        return self.model.pred_face

    def recon_coeff_vis_from_noise(self, batched_images, batched_lm5, return_image=True, batch_mode=True):
        bs = batched_images.shape[0]
        data_lst = []
        for i in range(bs):
            img = batched_images[i]
    ...
    @torch.no_grad()
    def recon_coeff_2(self, batched_images, batched_lm5, return_image=True, batch_mode=True):
        bs = batched_images.shape[0]
        data_lst = []
        for i in range(bs):
            img = batched_images[i]
